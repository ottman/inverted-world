...
function socialThread(
  lead: ClaimDossier | undefined,
  editionHeadline: string,
  editionDeck: string,
  sourceCount: number,
  siteUrl: string,
) {
  if (!lead) {
    return [
      `${editionHeadline}\n\n${editionDeck}`,
      `Start with ${sourceCount} source links, then move through the live desk and Tales archive.`,
      `Read the latest desk: ${absoluteUrl("/news", siteUrl)} / Archive: ${absoluteUrl("/archive", siteUrl)}`,
    ]
  }
  ...
  return [
    ...
    `${sourceLine}. ${archiveLine}. Read the story: ${absoluteUrl(`/news/${lead.slug}`, siteUrl)}`,
  ]
}
...
export async function buildDailyAutopostPacket(siteUrl = process.env.INVERTED_WORLD_SITE_URL || DEFAULT_SITE_URL) {
  ...
  const leadUrl = lead ? absoluteUrl(`/news/${lead.slug}`, siteUrl) : absoluteUrl("/news", siteUrl)
  const headlineList = headlineVariants(lead, headline)
  const thread = socialThread(lead, headline, deck, sources.length, siteUrl)
  const imagePrompts = [
  ...
  const links = {
    ...
    archiveVideos: archiveVideos.map((item) => ({
      title: textField(item.title),
      url: absoluteUrl(textField(item.href) || "/archive", siteUrl),
...
export async function buildDailyAutopostJobResult() {
  const packet = await buildDailyAutopostPacket()
  ...
  return {
    ...
    headline: packet.edition?.headline || null,
    canonicalUrl: packet.autopost.canonicalUrl,
    sourcePackCount: packet.readiness.sourcePackCount,
    headlineVariantCount: packet.readiness.headlineVariantCount,
    xThreadCount: packet.readiness.xThreadCount,
    imagePromptCount: packet.readiness.imagePromptCount,
    articleLinkCount: packet.readiness.articleLinkCount,
    xSignalLinkCount: packet.readiness.xSignalLinkCount,
    archiveVideoLinkCount: packet.readiness.archiveVideoLinkCount,
  }
}
