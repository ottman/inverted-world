...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  const staticRoutes: MetadataRoute.Sitemap = ["", "/archive", "/news", "/documents", "/how-it-works", "/llms.txt"].map((path) => ({
    url: `${baseUrl}${path}`,
    lastModified: new Date(),
    changeFrequency: "hourly",
    priority: path === "" ? 1 : path === "/news" ? 0.94 : path === "/archive" ? 0.92 : path === "/documents" ? 0.82 : path === "/how-it-works" ? 0.7 : 0.62,
  }))
