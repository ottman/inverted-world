...
For content products, include at least one machine-readable breadth gate before DNS. Examples: a news API with minimum item count, source mode, direct source links, and no boilerplate/template bodies; an archive API with category coverage; a media-library item route; or a freshness endpoint tied to the latest publishing job.
