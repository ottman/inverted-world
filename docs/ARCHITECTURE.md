...
- Worldwire/news board: Recursiv jobs crawl Exa, Brave, and RSS fallbacks into `coverage_snapshots` with `source = 'worldwire'`. `/news` reads those persisted snapshots, published article drafts, and claim dossier sources; public page renders do not call third-party crawler APIs directly.
