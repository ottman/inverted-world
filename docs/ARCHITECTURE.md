...
- Provider fallback policy: public request paths explicitly pass `allowProviderFallbacks: false` and read Recursiv database rows, committed Recursiv snapshots, curated static rows, or seed data. Recursiv job handlers explicitly opt into provider fallbacks for ingestion.
