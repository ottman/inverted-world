...
- Provider fallback policy: public request paths explicitly pass `allowProviderFallbacks: false` and read Recursiv database rows, committed Recursiv snapshots, curated static rows, or seed data. `pnpm audit:public-providers` enforces that public `app/` routes and pages do not call provider-capable helpers without the explicit public-read guard. Recursiv job handlers explicitly opt into provider fallbacks for ingestion.
...
- Source and media ingestion can still mirror official release documents, videos, audio, images, archive hubs, source-chain briefs, and research questions into Recursiv for story generation. The standalone public shelves are removed for now: `/documents`, `/api/documents`, `/media`, and `/api/media` must return 404 until those experiences are rebuilt around a clearer news flow.
...
- `scripts/provision-recursiv-backend.mjs --with-jobs` is the desired-state manifest for the Recursiv scheduled jobs, including hourly worldwire/front-page refresh, a six-hour bounded full-pipeline refresh, and a 30-minute pipeline-maintenance cleanup job. Its job endpoints use `async=1` to prevent scheduler-level timeout noise on ingestion tasks that intentionally keep running in the hosted app;
- `/api/autopost/daily` now exposes the Recursiv-published daily packet for site/social/newsletter/video reuse without exposing provider keys to the browser, and `/api/recursiv/jobs/daily-autopost` gives Recursiv cron an authenticated proof/build target for that packet;
