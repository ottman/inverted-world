...
- Pipeline runs: `pipeline_runs` records full-pipeline executions and step results. `/api/recursiv/jobs/full-pipeline` now marks old interrupted runs as `stale_running`, records current-step progress, and can run archive sync, topic pulse, dossiers, article generation, image generation, publishing, and front-page edition publication as one Recursiv-triggerable sequence.
