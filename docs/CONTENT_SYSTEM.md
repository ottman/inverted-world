...
Run `pnpm recursiv:snapshot:status` when the hosted database key is unhealthy or the deploy API is cooling down. It reports local snapshot row counts, the latest exported full-pipeline completion time, the two-hour freshness window for the hourly news desk, whether a protected direct database URL is available, and whether the Recursiv database API can actually list/query the configured database. A local Recursiv key source is not enough for refresh; only use `--source=recursiv-api` when the status output marks the Recursiv database query path usable. If list succeeds but credentials return `credentials_not_found` and `SELECT 1` returns `query_failed`, run `pnpm recursiv:db:probe` and repair the Recursiv database/Neon credentials path or add a protected direct database URL before refreshing.
