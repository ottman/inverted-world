...
- Source and media ingestion can still mirror official releases, primary-source PDFs, hearing records, agency files, videos, and extracted text into Recursiv for story generation, but the public `/documents`, `/api/documents`, `/media`, and `/api/media` shelves are removed until they can return as a clearer news experience.
...
`recursiv:provision -- --with-jobs` is the desired-state scheduled-job manifest. The public news board stays current through an hourly worldwire crawl followed by an hourly front-page edition build. The expensive full pipeline runs every six hours, with AI/article/image/media/archive work staggered on three-, six-, or twelve-hour cadences so the hosted Recursiv API key keeps daily headroom for public reads and operator proof. It points Recursiv cron at the hosted job endpoints with `async=1` so long ingestion tasks acknowledge quickly and do not leave scheduler `last_error` noise while the app continues work. Direct operator proof should call the same endpoint without `async=1` when the full JSON result is needed.
...
Run `pnpm recursiv:snapshot:status` when the hosted database key is unhealthy or the deploy API is cooling down. It reports local snapshot row counts, the latest exported full-pipeline completion time, the eight-hour full-pipeline freshness window, whether a protected direct database URL is available, and whether the Recursiv database API can actually list/query the configured database. A local Recursiv key source is not enough for refresh; only use `--source=recursiv-api` when the status output marks the Recursiv database query path usable. If list succeeds but credentials return `credentials_not_found` and `SELECT 1` returns `query_failed`, run `pnpm recursiv:db:probe` and repair the Recursiv database/Neon credentials path or add a protected direct database URL before refreshing.
...
`--provider=all` combines paid X API search, Exa-indexed X discovery, and the public profile-reader fallback. If paid X/Exa keys are not available locally, `--provider=all` still runs the profile-reader lane instead of failing. Use `--provider=x`, `--provider=exa`, or `--provider=profile` to isolate a single lane during proof.
...
`recursiv:deploy` and `recursiv:deploy:status` record a local cooldown in `/private/tmp/inverted-world-recursiv-api-cooldown.json` after a Recursiv API `429`, then refuse further deploy/status calls until the window is likely clear. If Recursiv reports a per-day limit, treat the cooldown as a real daily-budget blocker unless a healthy project/org key is available; older one-hour cooldown files with a per-day message are normalized to a 24-hour guard from their recorded time and rewritten locally so the file matches the active guard. Use `pnpm recursiv:deploy:window` for a no-call readiness check that prints the current commit, custom-domain payload, next allowed retry time, next command, and snapshot preflight without calling Recursiv. Use `node scripts/deploy-recursiv.mjs --clear-cooldown` only when the window has likely cleared, or `--ignore-cooldown` for a deliberate one-shot override. Actual deploy calls also check the committed Recursiv snapshot before making the API request and refuse when the snapshot freshness gate is already failed; use `--skip-snapshot-preflight` only when another proof path has established fresh Recursiv-backed data. `recursiv:deploy:dry-run` prints the intended non-secret payload and cooldown advice without calling Recursiv. `recursiv:deploy:custom-domain` redeploys production with `custom_domain=www.inverted.world` to create the pre-DNS custom-domain binding; `recursiv:deploy:custom-domain:wait` does the same and polls deployment status to completion. It must be followed by `recursiv:cutover` proof before any DNS record is changed.
